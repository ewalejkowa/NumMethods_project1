...
double blad_wzgledny(double a1, double a2){
	return abs((a1 - a2) / a1);
}

double potega(double liczba, int potega){  //funckja wyliczajaca potege liczby
	double pom = liczba;
...
double wzor_od_poczatku(double x, int *n, double epsilon){
	double wynik = 0;
	int i;
...
double wzor_od_konca(double x, int n, double epsilon){
	double wynik = 0;
...
double pop_od_poczatku(double x, int *n, double epsilon){
	double tmp = 1;
	double wynik = 0;
	int i;
...
double pop_od_konca(double x, int n, double epsilon){
	double wynik = 0;
	for (int i = n; exp(x) - wynik>epsilon && i>0; i--){
		wynik = wynik*x / i + 1;
	}
